...
# MODEL FITTING: Seasonality with sin cosine
# ------------------------------------------------------------------------------
...
#This Stan model fits the acquisition rate (1 → 2) using a sinusoidal function:
  # λ₁₂(t) = a₁·sin(2πX) + b₁·cos(2πX), where X is standardized time.
...
# Seasonal predictor (sine + cosine for consistency with Stan)
Y_hat_1_2 <- true_params$a1 * sin(2 * pi * X) + true_params$b1 * cos(2 * pi * X)

# Check if seasonal pattern
plot(c(1:length(Y_hat_1_2)), Y_hat_1_2)
...
lambda_df <- data.frame(
  Time = as.Date(X_midpoints, origin = "1970-01-01"),
  Lambda12 = Y_hat_1_2
)

ggplot(lambda_df, aes(x = Time, y = Lambda12)) +
  geom_line(color = "steelblue") +
  geom_point(alpha = 0.5) +
  labs(
    title = "Seasonal Component of Infection Rate",
    x = "Time",
    y = expression(lambda[12] ~ "(Seasonal Effect)")
  ) +
  theme_minimal()
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    if (n == 1 || menage_id_member[n] != menage_id_member[n - 1]) {
      ...
      log_lambda_1_2_out[n] = log12 + Y_hat_1_2_out[idx_first[n]];
    ...
    if (first_subinterval[n] > 0) {
      ...
      if (t0 < t_star1 && t_star1 < t1) {
        ...
        P_total *= transition_matrix(d1a, exp(log12 + s12), exp(log21));
        int flag1 = 1;
        int flag2 = ((t_star1 + t1) / 2 >= t_star2) ? 1 : 0;
        P_total *= transition_matrix(d1b, exp(log12 + flag1 * beta_int1_1 + flag2 * beta_int1_2 + s12),
                                          exp(log21 + flag1 * beta_int2_1 + flag2 * beta_int2_2));
      } else {
        int flag1 = ((t0 + t1) / 2 >= t_star1) ? 1 : 0;
        int flag2 = ((t0 + t1) / 2 >= t_star2) ? 1 : 0;
        P_total *= transition_matrix(first_subinterval[n], exp(log12 + flag1 * beta_int1_1 + flag2 * beta_int1_2 + s12),
                                                   exp(log21 + flag1 * beta_int2_1 + flag2 * beta_int2_2));
      }
    }
    ...
    for (m in 1:num_middle_subintervals[n]) {
      ...
      if (t0m < t_star1 && t_star1 < t1m) {
        ...
        P_total *= transition_matrix(d2a, exp(log12 + s12m), exp(log21));
        int flag1 = 1;
        int flag2 = ((t_star1 + t1m) / 2 >= t_star2) ? 1 : 0;
        P_total *= transition_matrix(d2b, exp(log12 + flag1 * beta_int1_1 + flag2 * beta_int1_2 + s12m),
                                          exp(log21 + flag1 * beta_int2_1 + flag2 * beta_int2_2));
      } else {
        int flag1 = ((t0m + t1m) / 2 >= t_star1) ? 1 : 0;
        int flag2 = ((t0m + t1m) / 2 >= t_star2) ? 1 : 0;
        P_total *= transition_matrix(interval_length, exp(log12 + flag1 * beta_int1_1 + flag2 * beta_int1_2 + s12m),
                                                    exp(log21 + flag1 * beta_int2_1 + flag2 * beta_int2_2));
      }
    }
    ...
    if (last_subinterval[n] > 0) {
      ...
      if (t0l < t_star2 && t_star2 < t1l) {
        ...
        P_total *= transition_matrix(d3a, exp(log12 + beta_int1_1 + s12l), exp(log21 + beta_int2_1));
        P_total *= transition_matrix(d3b, exp(log12 + beta_int1_1 + beta_int1_2 + s12l),
                                          exp(log21 + beta_int2_1 + beta_int2_2));
      } else {
        int flag2 = ((t0l + t1l) / 2 >= t_star2) ? 1 : 0;
        P_total *= transition_matrix(last_subinterval[n], exp(log12 + beta_int1_1 + flag2 * beta_int1_2 + s12l),
...
#ggsave(file = "./Output/Figures/Simulated_data/samplingtimes_villages_sim_seasonality.png", 
#       plot = combined, width = 20, height = 13) 

# SEE IF THE SEASONAL PATTERN IS PRESENT IN THE DATASET
# Summarize prevalence across all dates (control group only)
# Convert Y_hat_1_2 from log scale to natural scale
y_hat_df <- data.frame(
  Date = as.Date(X_midpoints, origin = "1970-01-01"),
  lambda12_seasonal = exp(Y_hat_1_2)  # natural scale)
) %>%
  mutate(month_year = floor_date(Date, "month"),
         subinterval = 1:length(X_midpoints)) 
...
# Merge on time
plot_df <- y_hat_df %>%
  left_join(prev_df) # Can ignore error, this is as links both with intervention and control the same seasonal pattern

plot_df_l <- plot_df%>%select(subinterval,Intervention, prevalence,lambda12_seasonal) %>%
  pivot_longer(cols = c(prevalence, lambda12_seasonal),
               names_to = "variable", values_to = "value") %>%
  mutate(variable = factor(variable,
                           levels = c("prevalence", "lambda12_seasonal"),
                           labels = c("Observed Prevalence", "Seasonal effect on λ12(t)")))

# Plot - for now just remove the subintervals where no samples were taken
p3 = ggplot(plot_df_l%>%filter(!is.na(Intervention)), aes(x = subinterval, y = value, color = variable, linetype = factor(Intervention))) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(
    title = "Observed Prevalence and Seasonal effect on λ12(t)",
    subtitle = "Control vs Intervention group",
    x = "Subinterval", y = "Value", color = "Measure", linetype = "Intervention"
  ) +
...
#ggsave(file = "./Output/Figures/Simulated_data/Cases_Seasonalpattern_sim_seasonality.png", 
...
"
#writeLines(stan_model_full, "./Scripts/Models_and_code_raneem/Latest/Simulations/stan_models/two_step_model_sine.stan")
...
saveRDS(file = "./Output/Validation_with_simulated_data/two_step_sine_season_simulated.rds")
