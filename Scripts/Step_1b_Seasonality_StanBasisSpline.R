...
#####Library loading 
## Use the coefficient wrapping trick for splines 
library(ggplot2)
library(rstan)
...
# Since X is in [-0.5, 0.5], knots should match this
knots <- seq(-0.5, 0.5, length.out = num_knots)
...
parameters {
  ...
  row_vector[num_basis - spline_degree] a_base_1_2;
  ...
  real<lower=0> sigma;
  real beta_int1;  // intervention effect on acquisition rate
  real beta_int2;  // intervention effect on decolonization rate
}
...
transformed parameters {
  ...
  // Infection spline: periodic coefficients
  row_vector[num_basis] a_1_2;

  for (i in 1:(num_basis - spline_degree)) {
    a_1_2[i] = a_base_1_2[i];
  }
  for (j in 1:spline_degree) {
    a_1_2[num_basis - spline_degree + j] = a_base_1_2[j];  // enforce periodicity
  }

  // Scaling and seasonal effect
  real tau_1_2 = exp(log_tau_raw_1_2);
  row_vector[num_basis] a_scaled_1_2 = a_1_2 * tau_1_2;
  real a0_1_2 = a0_raw_1_2 * sigma_a0_1_2;
  vector[num_data] Y_hat_1_2 = a0_1_2 * to_vector(X) + to_vector(a_scaled_1_2 * B);
}
...
generated quantities {
 ...
 // vector[num_data] Y_hat_2_1_out;
  ...
  row_vector[num_basis] a_1_2_out  = a_scaled_1_2;
 // row_vector[num_basis] a_2_1_out  = a_2_1;

  // Recompute seasonal splines
  Y_hat_1_2_out = a0_1_2 * to_vector(X) + to_vector(a_scaled_1_2 * B);
