...
intervention_effect <- "" # "" or "NonAdd_" or "NonAdd_NonCol_" or ""
...
summ <- bind_rows(
  bind_cols(label = "Pre-intervention HR (between groups)", summ_ci(pre_hr)),
  bind_cols(label = "Post-Intervention 1 (DiD)",           summ_ci(post1_hr)),
  bind_cols(label = "Post-Intervention 2 (DiD)",           summ_ci(post2_hr)),
  bind_cols(label = "Overall (Post1 & Post2)",             summ_ci(overall_hr))
) %>%
  mutate(
    label = factor(
      label,
