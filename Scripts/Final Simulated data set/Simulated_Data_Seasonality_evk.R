...
# total_days <- global_interval_end_num - global_interval_start_num + 1  # 549
# num_intervals <- ceiling(total_days / interval_length) # 20
# max_middle <- num_intervals - 2 # 18
...
#   first_subinterval_sim[idx[2]] <- pmin(interval_end_first - d1 + 1, days_12, interval_length)  # This should have been between d1 and global interval, not d1 and d2
#   total_days_23 <- d3 - d2 + 1
#   num_middle_subintervals_sim[idx[3]] <- pmax(0, ceiling(total_days_23 / interval_length) - 2) # I think what is missing here is all the mid intervals between 3 and 4, which is not only the last number of days outside the 28 day intervals
...
#----------------------------------------------------------------------------------------------------
# BELOW IS THE MODEL WHEN THE SPLIT FUNCTION SPLITS TRANSITIONS IN 28 DAY INTERVALS PER GLOBAL INTERVAL PER INDIVIDUAL
#----------------------------------------------------------------------------------------------------
stan_code <- "
functions {
...
"
#----------------------------------------------------------------------------------------------------
# BELOW IS THE MODEL WHEN THE SPLIT FUNCTION SPLITS TRANSITIONS IN 28 DAY INTERVALS PER TRANSITION
#----------------------------------------------------------------------------------------------------
# stan_code <- "
# functions {
#   matrix transition_matrix(real t, real lambda_1_2, real lambda_2_1) {
