...
model {
  ...
  for (i in 1:num_individuals) {
    acquisition_status[i, 1] ~ bernoulli_logit(logit(0.1));  // Prior for day 1
  }
  
  for (day in 2:num_days) {
    for (individual_number in 1:num_individuals) {
      if (acquisition_status[individual_number, day - 1] == 1) {
        acquisition_status[individual_number, day] ~ bernoulli_logit(logit(1.0);  // If already infected, keep them infected
      } else {
