...
# Sample data in wide format with named columns
wide_data <- data.frame(
  id = c(1, 1, 2, 2),
  time1 = c(0, 3, 0, 3),
  state1 = c("S", "I", "S", "I"),
  value1 = c(944, 1374, 6009, 20601),
  time2 = c(6, 9, 6, 9),
  state2 = c("S", "I", "S", "I"),
  value2 = c(6252, 20619, 6290, 20749)
)

# Correct column names
colnames(wide_data) <- c("id", "time1", "state1", "value1", "time2", "state2", "value2")
...
long_data <- wide_data %>%
  pivot_longer(
    cols = starts_with(c("time", "state", "value")),
    names_to = c(".value", "observation"),
    names_sep = ""
  ) %>%
  arrange(id, time)
