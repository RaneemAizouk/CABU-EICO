stan_data_fit <- list(
  ...
  N_persons = length(unique(simulated_dataset$MenagememberID)),
  H = length(unique(simulated_dataset$HouseID)),
  HouseID = simulated_dataset$HouseID,
  menage_id_member = simulated_dataset$MenagememberID,
  age = simulated_dataset$Age,
  round = simulated_dataset$Round,
  sexe = simulated_dataset$Sexe,
  observed_state = simulated_dataset$Observed_State_Sim,
  date_use = simulated_dataset$date_use_num,
  intervention = simulated_dataset$Intervention,
  intervention_date = as.numeric(simulated_dataset$Intervention_Date1 - as.Date("1970-01-01")),
  intervention_date2 = as.numeric(simulated_dataset$Intervention_Date2 - as.Date("1970-01-01")),
  global_interval_start = as.numeric(as.Date("2022-10-01") - as.Date("1970-01-01")),
  global_interval_end = as.numeric(as.Date("2024-04-01") - as.Date("1970-01-01")),
  interval_length = 28,
  num_data = length(stan_data$X),
  X = stan_data$X,
  max_middle = stan_data$max_middle,
  num_intervals = as.integer(stan_data$num_intervals),
  idx_first_sim = simulated_dataset$Idx_First_Sim,
  idx_last_sim = simulated_dataset$Idx_Last_Sim,
  idx_middle_sim = simulated_dataset$Idx_Middle_Sim,
  first_subinterval_sim = simulated_dataset$First_Subinterval_Sim,
  last_subinterval_sim = simulated_dataset$Last_Subinterval_Sim,
  num_middle_subintervals_sim = simulated_dataset$Num_Middle_Subintervals_Sim,
  global_interval_index_start = simulated_dataset$Global_Interval_Index_Start
)
names(stan_data_fit)

# Replace NA in intervention dates with -999 for control group
stan_data_fit$intervention_date[simulated_dataset$Intervention == 0] <- -999
stan_data_fit$intervention_date2[simulated_dataset$Intervention == 0] <- -999

# Ensure idx_middle is a matrix
if (!is.matrix(stan_data_fit$idx_middle)) {
  stan_data_fit$idx_middle <- matrix(stan_data_fit$idx_middle, nrow = stan_data_fit$N, ncol = stan_data_fit$max_middle)
}
  ...
  for (n in 1:N) {
    ...
    if (first_subinterval[n] > 0) {
      ...
      if (t0 < t_star1 && t_star1 < t1 && intervention[n] == 1) {
        ...
        real d1a = t_star2 - t0;
        real d1b = t1 - t_star2 + 1;
        log_lambda_1_2 = log12_base + s12 + beta_int1_1 + beta_int1_2; // Use post-intervention 2 rate
        log_lambda_2_1 = log21_base + beta_int2_1 + beta_int2_2;
        second_intervention_used[n] = 1;
      } else {
        real midpoint = (t0 + t1) / 2;
    ...
    for (m in 1:num_middle_subintervals[n]) {
      ...
      if (t0m < t_star1 && t_star1 < t1m && intervention[n] == 1) {
        ...
        real d2a = t_star2 - t0m;
        real d2b = t1m - t_star2 + 1;
        log_lambda_1_2 = log12_base + s12m + beta_int1_1 + beta_int1_2; // Use post-intervention 2 rate
        log_lambda_2_1 = log21_base + beta_int2_1 + beta_int2_2;
        second_intervention_used[n] = 1;
      } else {
        real midpoint = (t0m + t1m) / 2;
      ...
      log_lambda_1_2_out[n] = log_lambda_1_2; // Update with the latest rate
      log_lambda_2_1_out[n] = log_lambda_2_1;
    }
    ...
    if (last_subinterval[n] > 0) {
      ...
      if (t0l < t_star1 && t_star1 < t1l && intervention[n] == 1) {
        ...
        real d2a = t_star2 - t0l;
        real d2b = t1l - t_star2 + 1;
        log_lambda_1_2 = log12_base + s12l + beta_int1_1 + beta_int1_2; // Use post-intervention 2 rate
        log_lambda_2_1 = log21_base + beta_int2_1 + beta_int2_2;
        second_intervention_used[n] = 1;
      } else {
        real midpoint = (t0l + t1l) / 2;
