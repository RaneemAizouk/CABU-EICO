...
table(d_wash_r2$redcap_event_name)
...
length(unique(d_lab$menage_id_member[d_lab$redcap_event_name=="round_2_arm_1"])) # 1015
...
table(d_lab_de$redcap_event_name) # Was: 1191 individuals in round 0, new ones: 21 (round 1); 18 (round 2); 7 (round 3) 
...
table(dt$sexe, useNA="always")
View(dt%>%filter(is.na(sexe))) # 3 missing sexe

table(dt$date.stool.collection, useNA="always")

length(unique(dt$menage_id_member)) # 1237
length(dt$date) # 4601 samples
...
length(unique(dt$menage_id_member)) # 1237
length(unique(dt_filtered$menage_id_member)) # 1236
...
# Missing age and sex
table(dfls0$age, useNA="always")
table(dfls0$sexe, useNA="always")
...
length(unique(data$menage_id_member)) # 1236
...
missing_data_individuals <- data %>%
  ...
  filter(missing_age | missing_sexe)  # keep only those with a fully missing field - 16 observations
...
table(data_filtered$age, useNA="always")
length(unique(data_filtered$menage_id_member[is.na(data_filtered$sexe)]))
