...
true_params <- list(
  ...
  a1 = 1.5,                    # Amplitude of seasonal effect on λ₁₂
...
data {
  ...
  real X_seasonal[num_data];                        // Scaled time points (in years) for seasonal effect
...
generated quantities {
  ...
  for (i in 1:num_data) {
     Y_hat_1_2_out[i] = a1 * sin(2 * pi() * X_seasonal[i] + phi);  
  }
...
# Define groups
treated1_only <- simulated_dataset$Intervention == 1 &
  simulated_dataset$Intervention2_active == 0 &
  simulated_dataset$Intervention1_active == 1

treated2 <- simulated_dataset$Intervention == 1 &
  simulated_dataset$Intervention2_active == 1 &
  simulated_dataset$Intervention1_active == 1
...
safe_mean <- function(x) {
  if (sum(!is.na(x)) > 0) mean(x, na.rm = TRUE) else NA
}

# Compute group means safely
control_mean <- safe_mean(simulated_dataset$lambda_1_2[control])
treated1_mean <- safe_mean(simulated_dataset$lambda_1_2[treated1_only])
treated2_mean <- safe_mean(simulated_dataset$lambda_1_2[treated2])
...
# Verify observation timing between interventions
treated_data <- subset(simulated_dataset, Intervention == 1)
treated_data$in_between <- !is.na(treated_data$Intervention_Date2) &
  treated_data$Date >= treated_data$Intervention_Date1 &
  treated_data$Date < treated_data$Intervention_Date2

cat("Observations between 1st and 2nd intervention: ",
    sum(treated_data$in_between, na.rm = TRUE), "\n")

##########################

# Create date sequence for only one cycle
start_date <- as.Date("2022-10-01")
end_date <- as.Date("2024-02-01")  # 1-year range for one wiggle
date_seq <- seq(start_date, end_date, by = "day")

# Fractional year for one cycle
date_num <- as.numeric(date_seq - start_date) / 365
X <- date_num %% 1  # single cycle

# Interpolate seasonal effect for one cycle
seasonal_effect <- true_params$a1 * sin(2 * pi * X + true_params$phi)
...
# Plot seasonality
library(ggplot2)
p_seasonality <- ggplot(plot_data, aes(x = Date, y = Seasonal_Effect)) +
  geom_line(color = "blue") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  labs(x = "Date", y = "Seasonal Effect", title = "Annual Seasonal Effect on Acquisition Rate") +
