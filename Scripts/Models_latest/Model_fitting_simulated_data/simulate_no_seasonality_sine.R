...
# MODEL FITTING: Seasonality with sine cosine
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Fitting:
# Acquisition seasonality modeled using sinusoidal.
...
stan_data <- list(
  N = N,
  H = H,
  observed_state = as.integer(observed_state_sim),   # values: 1 or 2
  menage_id_member = simulated_dataset$menage_id_member,
  ...
  intervention_date = as.numeric(simulated_dataset$intervention_date),
  intervention_date2 = as.numeric(simulated_dataset$intervention_date2),
  num_data = length(X),
  X = X,  # standardized midpoint vector (e.g., centered years)
  max_middle = max_middle,
...
saveRDS(file = "./Output/Validation_with_simulated_data/two_step_sin_noseason_simulated.rds")
