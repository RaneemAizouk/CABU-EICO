...
# Of those with stool sample
dfls0 %>% filter(redcap_event_name=="round_0_arm_1") %>%
  group_by(intervention.text) %>%
  summarise(n = length(unique(menage_id))
  )

# Number of households per cluster - all households (also where only wash)
hh %>%
...
dfls0%>%
  filter(redcap_event_name == "round_0_arm_1") %>%
  group_by(intervention.text, menage_id) %>%
...
pdf("./Output/Figures_and_tables/BF/WASH_pre_post_diff.pdf", width=12,height=5)
multiplot(p, cols=1)
dev.off()

pdf("./Output/Figures_and_tables/BF/WASH_pre_post_abs_diff.pdf", width=7,height=5)
multiplot(p1, cols=1)
dev.off()
...
output_dir <- paste0("./Output/Figures_and_tables/BF/")
...
png("./Output/Figures_and_tables/BF/WASH_prevalence_pre.png",width = 6, height = 4, units = "in", res = 300)  # Start PNG device
multiplot(p5, cols = 1)  # Adjust 'cols' to control layout
dev.off()  # Close PNG device

png("./Output/Figures_and_tables/BF/WASH_prevalence_post.png",width = 6, height = 4, units = "in", res = 300)  # Start PNG device
