...
# PLOT INCIDENCE
#---------------------------------------------------------------------------------------------
plot_incidence_panel <- function(data_list, panel_title, panel_subtitle, upper_cap = 25, base_size = 16) {
    rates_draws <- data_list$rates_draws   # needs: Group, phase_label, ID12_per100
    ann_box     <- data_list$ann_box       # needs: Group, phase_label, hr_label, ypos
    
    # Display phases as "Pre / Post-Intervention 1 / Post-Intervention 2"
    recode_phase <- function(x) case_when(
      x %in% c("Pre-intervention","Baseline") ~ "Baseline",
      x %in% c("3-months\npost-intervention","Post-Intervention 1","Phase 1") ~ "Intervention phase 1\n~6 months",
      x %in% c("9-months\npost-intervention","Post-Intervention 2","Phase 2") ~ "Intervention phase 2\n~9 months",
      TRUE ~ as.character(x)
    )
    ...
    rates_draws <- rates_draws %>%
     mutate(
        ...
        phase_disp = factor(phase_disp,
                            levels = c("Baseline","Intervention phase 1\n~6 months","Intervention phase 2\n~9 months")),
        Group = factor(Group, levels = c("Intervention","Control")),
    ...
    fill_vals <- c(
      ...
      "Intervention.Intervention phase 2\n~9 months" = "#CB181D",
      "Control.Baseline"         = "#F0F0F0",
      "Control.Intervention phase 1\n~6 months"      = "#BDBDBD",
      "Control.Intervention phase 2\n~9 months"      = "#BDBDBD"
    )
...
data_s1$ann_box <- make_ann_box_from_hr_tbl(
    ...
    hr_tbl      = data_s1$hr_tbl,   # your HR table (Phase 1 / Phase 2)
    upper_cap   = 25,
