...
gillespe <- function(lambda1, lambda2, rho, S0, I0, tmax) {
  t <- 0 # initial time
 ...
 #Add
  #individuals <- numeric() # Initialise empty numeric vector to store individual states
  ...
  while (t < tmax) {
    ...
    if (ru < pf1/pf) {
      ...
      # add
      #individuals <- c(individuals, rep(1, S), rep(2, I))
    } else {
